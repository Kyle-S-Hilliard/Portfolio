...
class sca_run:
		...
		with open(fileName,'w') as f:
			...
			if self.cell_score_lists:
				...
				for cell_score_list in self.cell_score_lists:
					...
					f.write(''.join(['--------',cell_score_list,'_raw_scaled--------\n']))
					f.write(json.dumps(str(self.__count_cluster_scores(self.adata, ''.join([cell_score_list,'_raw_scaled'])))))
					f.write('\n')
	...
	def preprocess_data(self,adata):
		...
		if self.cell_score_lists:
			...
			sc.pp.scale(adata_score_temp, max_value=10)
			for file in self.cell_score_lists:
				score_list = [line.rstrip('\n') for line in open(''.join([file,'.txt']),'r')]

				sc.tl.score_genes(adata_score_temp, score_list, ctrl_size=200, gene_pool=None, n_bins=0, score_name=file+'_raw_scaled', random_state=0, copy=False, use_raw=False)
	...
	def plot_sca(self, adata, figdir='./figures/', adata_preFiltered=None,
		...
		if self.cell_score_lists:
			vmax = adata.obs.loc[:,[''.join([score_name,'_raw_scaled']) for score_name in self.cell_score_lists]].values.max()
			vmin = adata.obs.loc[:,[''.join([score_name,'_raw_scaled']) for score_name in self.cell_score_lists]].values.min()
			sc.pl.umap(adata, color=[''.join([score_name,'_raw_scaled']) for score_name in self.cell_score_lists], 
					   save='_cellType_score_raw_scaled.png', show=False, edges=False, color_map=my_feature_cmap, 
					   size=size, vmin=vmin, vmax=vmax)
			sc.pl.umap(adata, color=[''.join([score_name,'_raw_scaled']) for score_name in self.cell_score_lists], 
					   save='_cellType_score_raw_scaled_0min.png', show=False, edges=False, color_map=my_feature_cmap, 
					   size=size, vmin=0, vmax=vmax)

			sc.pl.violin(adata,[''.join([score_name,'_raw_scaled'])for score_name in self.cell_score_lists], 
